...
public class WebServer {
	...
	*/
	public WebServer(final TemplateEngine templateEngine, final Gson gson, PlayerLobby playerLobby, GameCenter gameCenter, ReplayList replayList) {
		// validation
		Objects.requireNonNull(templateEngine, "templateEngine must not be null");
		Objects.requireNonNull(gson, "gson must not be null");
		Objects.requireNonNull(playerLobby, "playerLobby must not be null");
		Objects.requireNonNull(replayList, "replayList must not be null");
	...
	*/
	public void initialize() {
		...
		get("/", new GetHomeRoute(templateEngine, playerLobby, replayList));

		// Show a sign in page
		get("/signin", new GetSignInRoute(templateEngine));

		get("/game", new GetGameRoute(gson, templateEngine, gameCenter, replayList));

		// Replay mode
		get("/replay/game", new GetReplayRoute(gson, templateEngine, gameCenter, replayList));

		get("/replay/stopWatching", new GetReplayExitRoute());


		// MARK: POST Requests

		post("/signin", new PostSignInRoute(templateEngine, playerLobby));

		post("/startgame", new PostGameRoute(playerLobby, gameCenter));

		post("/validateMove", new PostValidateMoveRoute(gson));

		post("/submitTurn", new PostSubmitTurnRoute(gson, gameCenter));

		post("/backupMove", new PostBackupMoveRoute(gson));

		post("/checkTurn", new PostCheckTurnRoute(gson, gameCenter));

		post("/resignGame", new PostResignGameRoute(gson, gameCenter));

		post("/replay/nextTurn", new PostReplayNextTurnRoute(gson, replayList, gameCenter));

		post("/replay/previousTurn", new PostReplayPreviousTurnRoute(gson, replayList, gameCenter));

		post("/signout", new PostSignOutRoute(playerLobby));
